...
async def create_user(user: dict):
    try:
        ...
        response_content = {"stateCode": "CRUD-002", "message": "Decode Error"}
    ...
    try:
        ...
        if os.path.exists(csv_file_path):
            try:
                ...
                response_content = {"stateCode": "CRUD-005", "message": "CSV file error"}
                await send_to_queue(None, props, response_content)
...
async def update_user(user: dict):
    ...
    # 파일 존재 여부 확인
    if not os.path.exists(csv_file_path):
        response_content = {"stateCode": "CRUD-003", "message": "File open fail"}
        await send_to_queue(None, props, response_content)
    ...
    if not updated:
        response_content = {"stateCode": "CRUD-007", "message": "CSV write error"}
        await send_to_queue(None, props, response_content)
...
async def delete_user(user: dict):
    ...
    if os.path.exists(csv_file_path):
        try:
            ...
            if not row_deleted:
                response_content = {"stateCode": "CRUD-008", "message": "User Not Exists"}
